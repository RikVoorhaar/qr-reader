...
def finder_soft_template(t_samples: np.ndarray, m: float, sigma: float = 1.0) -> np.ndarray:
    """Build a soft binary template for a finder-pattern cross-section.

    The ideal finder has bands:
        |t| ≤ 0.5m:  dark  (central dark square, index 0)
        0.5m < |t| ≤ 1.5m: light (inner gap, index 1)
        1.5m < |t| ≤ 2.5m: dark  (inner dark ring, index 2)
        2.5m < |t| ≤ 3.5m: light (outer white ring, index 3)
        |t| > 3.5m:   dark  (background/quiet zone, index 4)

    Soft transitions via tanh((|t| − boundary) / sigma).
    """
    template_idx = [0, 1, 0, 1, 0]  # dark=0, light=1 for each band
    boundaries = np.array([0.5, 1.5, 2.5, 3.5]) * m

    abs_t = np.abs(t_samples)
    # Start from the outermost band (dark) and layer inward
    result = np.zeros_like(t_samples, dtype=np.float64)  # outermost = index 4 = 0
    for idx_val, b in reversed(list(zip(template_idx, boundaries))):
        inside = 0.5 * (1.0 + np.tanh((b - abs_t) / sigma))
        result = result * (1.0 - inside) + idx_val * inside
    return result


def fit_m_single_ray(
    t_samples: np.ndarray,
    profile: np.ndarray,
    m_est: float,
    sigma: float = 1.0,
) -> dict:
    """Find the optimal module pitch *m* for one radial profile.

    Minimises MSE between the normalised profile and the soft template.

    Returns
    -------
    dict
        Keys: ``m_fitted``, ``mse``, ``success``.
    """
    ...
    def cost(m_val):
        template = finder_soft_template(t_valid, m_val, sigma)
        return float(np.mean((template - p_valid) ** 2))

    result = minimize_scalar(
        cost,
        bounds=(m_est * 0.5, m_est * 2.0),
        method="bounded",
    )
    return {
...
def fit_all_rays(
    ...
    """Fit *m* independently to each ray in a profile array.

    Returns
    -------
    m_fitted : ndarray (num_rays,)
    ...
    n_full = profiles.shape[1]
    centre_idx = n_full // 2
    t_samples = np.linspace(-max_dist, max_dist, n_full) - 0.0  # 0 = centre

    m_fitted = np.full(n_rays, np.nan)
    mse = np.full(n_rays, np.nan)
    success = np.full(n_rays, False)

    for i in range(n_rays):
        result = fit_m_single_ray(t_samples, profiles[i], m_est, sigma)
        m_fitted[i] = result["m_fitted"]
        mse[i] = result["mse"]
        success[i] = result["success"]

    return m_fitted, mse, success
...
for ci in show_indices:
    ...
    diag_half = 0.5 * np.hypot(W_roi, H_roi)
    max_dist = RAY_LENGTH * diag_half
    m_fitted, mse, success = fit_all_rays(profiles_norm, m_est, max_dist, sigma=1.0)

    ok = success & np.isfinite(m_fitted)
    m_median = float(np.median(m_fitted[ok])) if np.any(ok) else m_est
    print(f"\nCluster {ci}: m_est={m_est:.2f}px, m_fitted median={m_median:.2f}px  "
          f"({int(np.sum(ok))}/{len(m_fitted)})")
    ...
    # Overlay fitted m markers: dots at ±k*m for k ∈ {0.5, 1.5, 2.5, 3.5}
    # ── m itself (module pitch) as connected line ──
    m_x_pos = np.full(len(angles_deg), np.nan)
    m_x_neg = np.full(len(angles_deg), np.nan)
    for i in range(len(angles_deg)):
        if np.isfinite(m_fitted[i]):
            m_x_pos[i] = +m_fitted[i]
            m_x_neg[i] = -m_fitted[i]
    ax_heatmap.plot(m_x_pos, angles_deg, "-", color=C_E2, linewidth=1.5, alpha=0.5, label="m")
    ax_heatmap.plot(m_x_neg, angles_deg, "-", color=C_E2, linewidth=1.5, alpha=0.5)

    # ── Outer boundary at ±3 ──
    outer_pos = 3 * m_fitted
    outer_neg = -3 * m_fitted
    ax_heatmap.plot(outer_pos, angles_deg, "o-", color=C_GOOD, markersize=3, linewidth=1.5, label="3.5m")
    ax_heatmap.plot(outer_neg, angles_deg, "o-", color=C_GOOD, markersize=3, linewidth=1.5)
