...
class CandidateCluster(NamedTuple):
    row: float
    cols: np.ndarray  # shape (6,)
    height: float
    num_candidates: int


rows_cand = np.array(rows_x[valid], dtype=np.float32)
cols_cand = np.array(cols_x_all[valid], dtype=np.float32)
height_cand = np.array(cols_x_all[valid, 3] - cols_x_all[valid, 2], dtype=np.float32)
...
def candidate_lengths_match(
    ...
    return np.abs(log_length1 - log_length2) < length_thresh
...
def choose_ref_candidate(candidates, processed_mask, rng):
...
while not processed_mask.all():
    ref_candidate, processed_mask, rng = choose_ref_candidate(
        candidates, processed_mask, rng
    )
    # Iterate over all candidates, merging those that overlap with ref_candidate.
...
region_mask = np.zeros_like(img_binary, dtype=bool)
...
# NumPy-friendly flood fill: expand the wave front to 8-connected neighbors in one step (vectorized).
...
# %%
input_img = np.asarray(img_binary)
