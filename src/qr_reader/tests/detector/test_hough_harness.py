...
def _edge_normal_from_points(a: np.ndarray, b: np.ndarray) -> tuple[np.ndarray, float]:
    """Compute unit normal and rho (≥ 0) for the infinite line through a→b."""
    d = b - a
    ...
    direction = d / length
    # Normal perpendicular to direction: (dy, -dx)
...
def _compute_finder_edges(
    ...
    """Compute finder-pattern boundary edges from synth-pipeline metadata.

    Given the ``corners_qr`` (TL, TR, BR, BL in image x/y coords) and ``N``
    (modules per side) from *metadata*, returns the 8 finder‑pattern boundary
    edges.

    If *roi_offset* / *roi_shape* are provided, segment endpoints are
    converted to ROI‑local coordinates and clipped.  Otherwise, everything
    stays in full‑image coordinates.

    Each returned dict has keys:
        label        — "TL_top", "TL_left", …
    ...
    # Each entry: (start_corner, toward_corner, label)
    edge_specs = [
        (TL, TR, "TL_top"),
        (TL, BL, "TL_left"),
        (TR, TL, "TR_top"),
    ...
    for start, toward, label in edge_specs:
        ...
        if roi_offset is not None and roi_shape is not None:
            ...
            # Convert to ROI-local (x=col, y=row)
            offset_xy = np.array([col0, row0], dtype=np.float64)
            a_local = a - offset_xy
            b_local = b - offset_xy

            # rho in ROI-local: normal · p_local = rho - normal · offset
...
class TestFinderEdges:
    ...
    def test_clean_axis_aligned_qr(self):
        ...
        # Find TL_top
        tl_top = next(e for e in edges if e["label"] == "TL_top")
    ...
    def test_all_edges_present(self):
        """All 8 expected edges are returned."""
        corners = {
            "TL": [10.0, 10.0],
            "TR": [100.0, 20.0],
            "BR": [90.0, 110.0],
