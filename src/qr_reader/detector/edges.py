...
def hysteresis_link(
    ...
    """8-connected flood from strong (> high percentile) to weak (> low percentile) edge pixels.

    Mimics Canny's hysteresis step: preserves weak edge pixels connected to
    strong ones, suppressing isolated noise.  Returns a boolean mask of linked
    pixels usable as a support-collection gate in ``refine_line``.
    ...
    angle : ndarray, shape (H, W)
        Edge-normal angles (unused, kept for signature compatibility).
    high_pct : float
        Percentile for strong-edge threshold.  Default 90.
    low_pct : float
        Percentile for weak-edge threshold.  Default 70.

    Returns
    -------
    linked : ndarray[bool], shape (H, W)
        Boolean mask where ``True`` means the pixel is connected to a strong edge.
    """
    from scipy.ndimage import label as nd_label  # noqa: PLC0415

    vals = nms[nms > 0]
    if len(vals) == 0:
        return np.zeros_like(nms, dtype=bool)

    high_thresh = float(np.percentile(vals, high_pct))
    low_thresh = float(np.percentile(vals, low_pct))

    strong = nms >= high_thresh
    weak = (nms >= low_thresh) & (~strong)

    structure = np.ones((3, 3), dtype=bool)
    labeled, n_components = nd_label(strong | weak, structure=structure)

    strong_labels = set(labeled[strong].ravel())
    linked = np.isin(labeled, list(strong_labels))
